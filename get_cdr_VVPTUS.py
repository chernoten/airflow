...
def get_cdr():
    ftp = ftplib.FTP('172.31.161.2')
    ftp.login('RCUUSE', 'RCUUSER')
    ftp.cwd('VIDAST')
    ls = ftp.retrlines('LIST ' + 'CF*', get_cdr_name)
    #print(f'last_cdr is {last_cdr}')
    server_time =  datetime.now().strftime("%d-%m-%Y_%I-%M-%S")
    local_cdr = open("dags/radio-sorm/data/VVPTUS/in/{}_".format(server_time) + last_cdr, 'wb')
...
def get_cdr_name(info):
    ...
    if re.match(r'CF\d{4}.D00', ls_out[-1]):
        ...
        if td <= timedelta(minutes=6000):
            global last_cdr
            last_cdr = ls_out[-1]
    return None

dag = DAG('CDR_Parser_VVTUS', description='get and parse CDR',
...
parser_operator =  BashOperator(
        task_id= 'parser_task',
        bash_command='python ~/dags/radio-sorm/sorm/main.py --ptus=VV ~/{{ task_instance.xcom_pull(task_ids="get_CDR") }}',
        dag=dag
)
